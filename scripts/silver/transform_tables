...
BEGIN
	...
	BEGIN TRY
        ...
        SELECT 
            ...
            END AS sls_due_dt,
            sls_quantity,
            CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales <=0 OR sls_sales != sls_quantity * ABS(sls_price) 
        ...
        cat,
        subcat,
        CASE WHEN TRIM(maintenance) LIKE 'Yes%' THEN 'Yes'
            WHEN TRIM(maintenance) LIKE 'No%' THEN 'No'
        END AS maintenance
